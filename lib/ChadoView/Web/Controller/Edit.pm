package ChadoView::Web::Controller::Edit;

=head1 NAME

ChadoView::Web::Controller::Edit - controller to handler /edit/... requests
...
You can find documentation for this module with the perldoc command.
    ...
    perldoc ChadoView::Web::Controller::Edit
...
sub _get_field_values
{
  ...
  if (defined $values_constraint) {
    ...
    if ($values_constraint =~ /$pattern/) {
      $constraint_path = ChadoView::DBLayer::Path->new(path_string => $1);
      $constraint_value = $2;
    } else {
...
sub _initialise_form
{
  ...
  for my $field_info (@field_infos) {
    ...
    my $class_name = ChadoView::DB::class_name_of_table($type);
    ...
    if (defined $info_ref) {
      ...
      my $referenced_table = ChadoView::DB::table_name_of_class($referenced_class_name);
...
sub _create_object {
  ...
  my $class_name = ChadoView::DB::class_name_of_table($table_name);
